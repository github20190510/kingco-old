                        ...
                        <div class="row">
                            ...
                            <div class="col-md-10 middle text-left mt-2">
                                <div class="user-edit-module">
                                    <div class="mem-title">
                                        <h5 class="d-flex align-items-center"><span class="title-icon withdrawal-icon mr-2"></span><i class="fas fa-hand-holding-usd mr-2"></i> {{ trans_choice('msg_zy_1.withdraw', 1) }}</h5>
                                    </div>
                                    <div class="w-50">
                                        ...
                                        <div class="form-group row">
                                            <label for="branchBank" class="col-sm-3 col-form-label">{{ __('msg_zy_1.withdrawal') . __('msg_zy_1.amount') }}:</label>
                                            <div class="col-sm-9 my-1">
                                                <input type="number" class="form-control mem_input" id="branchBank" placeholder="{{ __('msg_zy_1.input_wd_amount') }}" min="0" v-model="get_money">
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            <label for="paymentPassword" class="col-sm-3 col-form-label">{{ __('msg_zy_1.pay_pwd') }}:</label>
                                            <div class="col-sm-9 my-1">
                                                <input type="password" class="form-control mem_input" id="paymentPassword" placeholder="{{ __('msg_zy_1.input_pay_pass') }}" v-model="pay_pwd">
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            <label for="paymentPassword" class="col-sm-3 col-form-label">{{ __('msg_zy_1.mb_comments') }}:</label>
                                            <div class="col-sm-9 my-1">
                                                <input type="text" class="form-control mem_input" id="paymentMsg" placeholder="{{ __('msg_zy_1.input_mb_comments') }}" v-model="pay_msg">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="savbtn w-50 text-right" v-if="bln_can_send == 0" style="color:red">
                                        {{ __('msg_zy_1.should_bet') }}：@{{int_should_money}} {{ __('msg_zy_1.already_bet') }}：@{{int_bet_money}}
                                    </div>
                                    <div class="savbtn w-50 text-right" v-if="bln_can_send == 0">
                ...
                get_money_account: function(){
                    ...
                    .then(function (response) {
                        ...
                        if(this.json['code'] == '1'){
                            ...
                            self.alertinfo('<?php echo __("msg_zy_1.waiting_review") ?>');
                            self.getbagmoney();
                        }else{
                            self.if_send_or_not = false;
