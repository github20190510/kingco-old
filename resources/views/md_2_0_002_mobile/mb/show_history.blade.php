    ...
    <body class="main-body">
        ...
        <div id="show_records">
            <main role="main" class="container main bank-main">
                ...
                <section class="mb-2">
                    <div class="row">
                        <div class="col text-left"><i class="far fa-list-alt"></i> {{ __('msg_zy_1.total_count') }}: <strong class="text-warning">@{{ data_count }}</strong></div>
                        <div class="col text-right"><i class="fas fa-coins"></i> {{ __('msg_zy_1.total_money') }}: <strong class="text-warning">@{{ formatMoney(data_bet) }}</strong></div>
                    </div>
                </section>
                <section class="funds-table mb-2" v-if="hasdata === 1" v-for="gamedata in gamedatas">
                    <div class="row funds-date">
                        <div class="col-7">{{ __('msg_zy_1.time') }} : <span>@{{ gamedata.betDay }}</span></div>
                        <div class="col-5 text-right">{{ __('msg_zy_1.total_amount') }} : <strong class="text-warning">@{{ gamedata.settleamount }}</strong></div>
                    </div>
                    <div class="row funds-1">
                        <div class="col">{{ __('msg_zy_1.note_no') }} : <span class="order">@{{ gamedata.orderId }}</span></div>
                    </div>
                    <div class="row funds-1">
                        <div class="col">{{ __('msg_zy_1.game') }} :<span>@{{ gamedata.gamePlatform }}</span>-<span>@{{ gamedata.gameBigType }}-<span>@{{ gamedata.gamename }}</span><!---<strong>游戏名</strong>--></div>
                    </div>
                    <div class="row funds-2">
                        <div class="col-6">{{ __('msg_zy_1.betting') . __('msg_zy_1.amount') }} : <strong class="text-warning">@{{ formatMoney(gamedata.betMoney) }}</strong></div>
                        <div class="col-6 text-right">{{ __('msg_zy_1.winning') }} : <strong :class="[gamedata.intwin == 1 ? 'text-win' : 'text-lose']">@{{ formatMoney(gamedata.netMoney) }}</strong></div>
                    </div>
                </section>
