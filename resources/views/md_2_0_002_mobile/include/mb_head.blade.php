        ...
        <div class="d-flex align-items-center">
            ...
            <div class="navbar-collapse offcanvas-collapse align-items-start" id="navbarsExampleDefault">
                <ul class="navbar-nav mr-auto">
                    <li class="nav-item" :class="{'active': login_status === '1'}"> <a class="nav-link" href="/mb/mb_index"><i class="fas fa-user-cog fa-fw"></i> {{ __('msg_zy_1.member_center') }}</a> </li>
                    <li class="nav-item" :class="{'active': login_status === '1'}"> <a class="nav-link" href="/mb/acc"><i class="fas fa-id-card fa-fw"></i> {{ __('msg_zy_1.mb_account') }}</a> </li>
                    <li class="nav-item" :class="{'active': login_status === '1'}"> <a class="nav-link" href="/mb/edata"><i class="fas fa-user-alt fa-fw"></i> {{ __('msg_zy_1.personal_inf') }}</a> </li>
                    <li class="nav-item" :class="{'active': login_status === '1'}"> <a class="nav-link" href="/mb/message"><i class="fas fa-bell fa-fw"></i> {{ __('msg_zy_1.my_info') }}</a> </li>
                    <li class="nav-item active"> <a class="nav-link" href="/helpers"><i class="fas fa-user-tie fa-fw"></i> {{ __('msg_zy_1.about_us') }}</a> </li>
                    <li class="nav-item" :class="{'active': login_status === '1'}"> <a class="nav-link" href="#" @click="logout('head', '{{ __('msg_zy_1.logout_succ') }}')"><i class="fas fa-sign-out-alt fa-fw"></i> <?php echo trans(__('msg_zy_1.logout2')) ?></a></li>
    ...
    <div class="user-bar d-flex align-items-center justify-content-between" v-if="login_status === '1'">
        <div class="col-10 flex-shrink-1">
            <div class="row">
                ...
                <span class="credit" id="mb_acc_money"><i class="fas fa-wallet fa-fw"></i> {{ __('msg_zy_1.balance') }} 0.00</span>
            </div>
        </div>
...
var mb_head = new Vue({
    ...
    data: {
        login_status: '0',
        nick_name : '',
        title: '<?php echo __("msg_zy_1.member_center"); ?>',
        msg_count : 0,
        login_acc : ''
    },
    created: function(){
        ...
        .then(function (response) {
            ...
            if(this.json['code'] == '1'){
                ...
                self.alertinfo('<?php echo __('msg_zy_1.auto_error'); ?>', '/mb/login_page');
                return;
    ...
    methods: {
        get_title: function(){
            ...
            switch(location.pathname.replace(/\\d*/g, '')){
                case '/mb/acc':
                    self.title = '{{ __("msg_zy_1.mb_account") }}';
                    break;
                case '/mb/epwd':
                    self.title = '{{ __("msg_zy_1.edit_login_pwd") }}';
                    break;
                case '/mb/eppwd':
                    self.title = '{{ __("msg_zy_1.edit_pay_pwd") }}';
                    break;
                case '/discount':
                    self.title = '{{ __("msg_zy_1.promotion") }}';
                    break;
                case '/discount_content':
                    self.title = '{{ __("msg_zy_1.promotion") }}';
                    break;
                case '/mb/bank_card':
                    self.title = '{{ __("msg_zy_1.bank_card") }}';
                    break;
                case '/mb/bank_card_add':
                    self.title = '{{ __("msg_zy_1.add_bank_card_mobile") }}';
                    break;
                case '/mb/edata':
                    self.title = '{{ __("msg_zy_1.personal_inf") }}';
                    break;
                case '/mb/deposit':
                    self.title = '{{ __("msg_zy_1.saving_money") }}';
                    break;
                case '/mb/withdraw':
                    self.title = '{{ __("msg_zy_1.geting_money") }}';
                    break;
                case '/mb/notic':
                    self.title = '{{ __("msg_zy_1.system").__("msg_zy_1.notification") }}';
                    break;
                case '/mb/message':
                    self.title = '{{ __("msg_zy_1.my_info") }}';
                    break;
                case '/mb/deposit_list':
                    self.title = '{{ __("msg_zy_1.deposit").__("msg_zy_1.record") }}';
                    break;
                case '/mb/withdraw_list':
                    self.title = '{{ trans_choice("msg_zy_1.withdraw", 1).__("msg_zy_1.record") }}';
                    break;
                case '/mb/details':
                    self.title = '{{ __("msg_zy_1.money_record") }}';
                    break;
                case '/mb/showrecord':
                    self.title = '{{ __("msg_zy_1.betting") . __("msg_zy_1.record") }}';
                    break;
            }
