...
<html lang="{{ app()->getLocale() }}">
        ...
        <div id="rtglist">
            <div class="container-fluid text-center slot-nav-bar">
                ...
                <!-- <div class="slot-type d-flex align-items-center justify-content-center">
                    ...
                    <button type="button" class="btn slot-type-btn">{{ __('msg_dt_1.prize_pool_slot_machine') }}</button>
                    ...
                    <button class="btn gb-gold text-white btn-sm ml-2" type="button" @click="search_data()">{{ __('msg_dt_1.search') }}</button>
                </div> -->
            </div>
            <div class="container slot-main">
                <div class="game-box d-flex flex-wrap align-content-start">
                    <div class="game" :class="{'disabled': v['status'] === false, 'enabled': v['status'] === true}" v-for="(v, k) in gamedatas">
                        <div class="mask"><a :href="'/game/intogame/RTGGame/' + v['gamecode']" target="_blank"><span>{{ __('msg_dt_1.enter') . __('msg_dt_1.game') }}</span></a></div>
                    ...
                    <ul class="pagination pagination-lg justify-content-center">
                        ...
                        <li class="page-item">
                            <select @change="select_go_page()" v-model="ddl_page" class="form-control mt-2 ml-2">
                                <option :value="v" v-for="v in maxPage">{{ __('msg_dt_1.page_first_word') }}@{{v}}{{ __('msg_dt_1.page_last_word') }}</option>
                            </select>
                        </li>
                    </ul>
                </nav>
    ...
    <script>
        ...
        readTextFile('{{asset("js/games/rtg_list.js")}}', function(text){

            json_data = JSON.parse(text);
            for_data = json_data['rtg_game_data'];
            var app = new Vue({
                el : '#rtglist',
                data : {
                    gamedatas : new Array(),
