                            ...
                            <div class="col-md-10 middle-history text-left mt-2">
                                <div class="user-edit-module">
                                    <div class="mem-data-title">
                                        <h5 class="d-flex align-items-center"><i class="far fa-file-alt mr-2"></i> {{ __('msg_dt_1.betting') . __('msg_dt_1.record') }}</h5>
                                    </div>
                                    <div>
                                        <div class="form-row align-items-center">
                                            ...
                                            <div class="col-auto my-1">
                                                <select class="custom-select custom-select-sm mr-sm-2" v-model="gtype" @change="change_game_type()">
                                                    <option value="nodata" selected>{{ __('msg_dt_1.game') . __('msg_dt_1.type') }}</option>
                                                    <option value="SX">{{ __('msg_dt_1.real_casino') }}</option>
                                                    <option value="DZ">{{ __('msg_dt_1.slots') }}</option>
                                                    <option value="TY">{{ __('msg_dt_1.sport') }}</option>
                                                    <option value="CP">{{ __('msg_dt_1.lottery') }}</option>
                                                    <option value="QP">{{ __('msg_dt_1.cardgame') }}</option>
                                                    <option value="BY">{{ __('msg_dt_1.fishing').__('msg_dt_1.game') }}</option>
                                                </select>
                                            </div>
                                            <div class="col-auto my-1">
                                                <div class="ant-calendar-picker-input ant-input-sm">
                                                    <vue-datepicker-local v-model="range" placeholder="{{ __('msg_dt_1.start') . __('msg_dt_1.date') }}"  tabindex="-1" style="color:initial" @input="change_date()" format="YYYY-MM-DD HH:mm:ss"></vue-datepicker-local>
                                                    <span class="ant-calendar-picker-icon"></span>
                                                </div>
                                            </div>
                                            <div class="col-auto my-1">
                                                <button type="submit" class="btn btn-warning btn-sm" @click="only_win()" v-if="onlywin === 0">{{ __('msg_dt_1.only_win') }}</button>
                                                <button type="submit" class="btn btn-warning btn-sm" @click="all_data()" v-if="onlywin === 1">{{ __('msg_dt_1.view_all') }}</button>
                                            </div>
                                        </div>
                                        <div>
                                            <div class="col-auto my-1 text-gold">
                                                {{ __('msg_dt_1.single_page_bet_money') }}：@{{record_page_bet_total}}&nbsp;&nbsp;&nbsp;&nbsp;{{ __('msg_dt_1.single_page_net_money') }}：@{{record_page_net_total}}&nbsp;&nbsp;&nbsp;&nbsp;{{ __('msg_dt_1.total_count') }}：@{{record_total_count}}&nbsp;&nbsp;&nbsp;&nbsp;{{ __('msg_dt_1.total_bet_money') }}：@{{record_bet_total}}&nbsp;&nbsp;&nbsp;&nbsp;{{ __('msg_dt_1.total_net_money') }}：@{{record_net_total}}
                                            </div>
                                        </div>
                                        <table class="table">
                                            <thead>
                                                <tr>
                                                    <th scope="col" width="10%">{{ __('msg_dt_1.platform') }}</th>
                                                    <th scope="col" width="15%">{{ __('msg_dt_1.note_no') }}</th>
                                                    <th scope="col" width="15%">{{ __('msg_dt_1.game') }}</th>
                                                    <th scope="col" width="10%">{{ __('msg_dt_1.type') }}</th>
                                                    <th scope="col" width="10%">{{ __('msg_dt_1.betting') . __('msg_dt_1.amount') }}</th>
                                                    <th scope="col" width="10%">{{ __('msg_dt_1.winning') }}</th>
                                                    <th scope="col" width="10%">{{ __('msg_dt_1.total_amount') }}</th>
                                                    <th scope="col" width="15%">{{ __('msg_dt_1.time') }}</th>
                                                </tr>
                                            </thead>
                                            <tbody ref="showList">
        ...
        var app = new Vue({
            ...
            data : {
                gamenames : [
                    {
                        id: 'nodata',
                        name: '<?php echo __("msg_dt_1.game") . __("msg_dt_1.platform"); ?>'
                    }
                ],
                gamedatas : [],
                range: [new Date(), new Date()],
                local : {
                    hourTip: '<?php echo __("msg_dt_1.sel_hours"); ?>',
                },
                gname: 'nodata',
                gtype: 'nodata',
                hasdata: 0,
                            ...
                            if(rtnData['count'] > 0){
                                ...
                                $.each(rtnData['record'], function(key, value){
                                    ...
                                    switch(value['gamebigtype']){
                                        case 'SX':
                                            bigType = '<?php echo __("msg_dt_1.real_casino"); ?>';
                                            break;
                                        case 'DZ':
                                            bigType = '<?php echo __("msg_dt_1.slots"); ?>';
                                            break;
                                        case 'TY':
                                            bigType = '<?php echo __("msg_dt_1.sport"); ?>';
                                            break;
                                        case 'CP':
                                            bigType = '<?php echo __("msg_dt_1.lottery"); ?>';
                                            break;
                                        case 'QP':
                                            bigType = '<?php echo __("msg_dt_1.cardgame"); ?>';
                                            break;
                                        case 'BY':
                                            bigType = '<?php echo __("msg_dt_1.fishing").__("msg_dt_1.game"); ?>';
                                            break;
                                    }
