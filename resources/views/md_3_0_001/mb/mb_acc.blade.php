                            ...
                            <div class="col-md-10 middle-history text-left mt-2">
                                <div class="user-edit-module">
                                    <div class="mem-info-title">
                                        <h5 class="d-flex align-items-center"><i class="fas fa-info-circle mr-2"></i> {{ __('msg_dt_1.my_assets') }}</h5>
                                    </div>
                                    <div class="summary-module">
                                                ...
                                                <ul>
                                                    <li class="total">
                                                        <div class="userItems account">
                                                            <div class="account-balance"><span class="label">{{ __('msg_dt_1.acc_bal') }}</span><span class="money money-1"> <strong class="h4" id="mb_acc_money">@{{ acc_money }}</strong> {{ __('msg_dt_1.dollars') }}</span><span class="money money-2" style="display: none;"> <strong class="h3">@{{ acc_money }}</strong> {{ __('msg_dt_1.dollars') }}</span></div>
                                                            <button type="button" id="deposit_btn" class="btn btn-danger btn-sm" onclick="location.href='{{ url('mb/deposit') }}'">{{ __('msg_dt_1.recharge') }}</button>
                                                            <button type="button" id="withdraw_btn" class="btn btn-light btn-sm" onclick="location.href='{{ url('mb/withdraw') }}'">{{ trans_choice('msg_dt_1.withdraw', 0) }}</button>
                                                            <button type="button" id="enabled_btn" class="btn btn-light btn-sm" @click="fast_get_money()">{{ __('msg_dt_1.one_click_out') }}</button>
                                                        </div>
                                                    </li>
                                                    <li class="platform" v-for="(game,index) in games">
                                                        ...
                                                        <div class="action" v-if="loading_game_finishing == 1">
                                                            <span class="enabled" data-toggle="modal" data-target="#set_money" :data-stuff="[game.name,game.gametype]" @click="set_game_index(index)">{{ __('msg_dt_1.trans_in') }}</span>
                                                            <span class="enabled" data-toggle="modal" data-target="#get_money" :data-stuff="[game.name,game.gametype]" @click="set_game_index(index)">{{ __('msg_dt_1.trans_out') }}</span>
                                                        </div>
                                                        <div class="mask" v-if="loading_game_finishing == 1">
                                                            <span class="enabled" @click="fast_set_money(game.gametype,index)">{{ __('msg_dt_1.flash_in') }}</span>
                                                        </div>
                                                    </li>
                                                </ul>
                                            </div>
                                        ...
                                        <div class="mem-info-title mt-5">
                                            <h5 class="d-flex align-items-center"><i class="fas fa-info-circle mr-2"></i> {{ __('msg_dt_1.my_inf') }} <a href="{{ url('mb/edata') }}"><span class="small ml-2 text-warning">{{ __('msg_dt_1.update') }}</span></a></h5>
                                        </div>
                                        <div class="row">
                                            <div class="content col-6 text-left memberName">
    											<div class="row">
    												<div class="col-4"><span class="inputname">{{ __('msg_dt_1.account') }}&nbsp;:</span></div>
    												<div class="col-8"><span class="membertxt">@{{ userinfo.loginaccount }}</span></div>
    											</div>
    											<div class="row"><!-- 原nickname -->
    												<div class="col-4"><span class="inputname">{{ __('msg_dt_1.real_name') }}&nbsp;:</span></div>
    												<div class="col-8"><span class="membertxt">{{ userinfo.displayalias != '' ? hide_string("name",userinfo.displayalias) : '<?php echo __("msg_dt_1.modified"); ?>'}}</span></div>
    											</div><!-- 原nickname -->
    											<!-- <div class="row">
    												<div class="col-4"><span class="inputname">{{ __('msg_dt_1.real_name') }}&nbsp;:</span></div>
    												<div class="col-8"><span class="membertxt">{{ __('msg_dt_1.modified') }}</span></div>
    											</div> -->
    											<div class="row">
                                            ...
                                            <div class="col-6 text-left memberPhoto">
                                                <div class="row">
    												<div class="col-4"><span class="inputname"><span class="memicon phone-icon"></span>{{ __('msg_dt_1.mobile') }} :</span></div>
    												<div class="col-8"><span class="membertxt">{{ userinfo.phoneno != '' ? hide_string("phone",userinfo.phoneno) : '<?php echo __('msg_dt_1.modified'); ?>'}}</span></div>
                                                </div>
                                                <div class="row">
    												<div class="col-4"><span class="memicon mail-icon"></span><span class="inputname">{{ __('msg_dt_1.email') }} :</span></div>
    												<div class="col-8"><span class="membertxt">{{ userinfo.email != '' ? hide_string("email",userinfo.email) : '<?php echo __('msg_dt_1.modified'); ?>'}}</span></div>
                                                </div>
                                                <div class="row">
    												<div class="col-4"><span class="memicon qq-icon"></span><span class="inputname">{{ __('msg_dt_1.qq') }} :</span></div>
    												<div class="col-8"><span class="membertxt">{{ userinfo.qq != '' ? hide_string("qq",userinfo.qq) : '<?php echo __('msg_dt_1.modified'); ?>'}}</span></div>
                                                </div>
                                                <div class="row">
    												<div class="col-4"><span class="memicon wechat-icon"></span><span class="inputname">{{ __('msg_dt_1.wechat') }} :</span></div>
    												<div class="col-8"><span class="membertxt">{{ userinfo.wechat != '' ? hide_string("wechat",userinfo.wechat) : '<?php echo __('msg_dt_1.modified'); ?>'}}</span></div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-4"><span class="memicon"></span><span class="inputname">{{ __('msg_dt_1.line') }} :</span></div>
                                                    <div class="col-8"><span class="membertxt">{{ userinfo.line != '' ? hide_string("line",userinfo.line) : '<?php echo __('msg_dt_1.modified'); ?>'}}</span></div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-4"><span class="memicon"></span><span class="inputname">{{ __('msg_dt_1.skype') }} :</span></div>
                                                    <div class="col-8"><span class="membertxt">{{ userinfo.skype != '' ? hide_string("skype",userinfo.skype) : '<?php echo __('msg_dt_1.modified'); ?>'}}</span></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-header text-white brown">
                                                <h5 class="modal-title" id="set_monty_title">{{ __('msg_dt_1.trans_in') }}</h5>
                                            ...
                                            <div class="modal-body mt-4">
                                                <div class="form-group row">
                                                    <label for="exampleInputPassword1" class="col-sm-3 col-form-label" style="color:black">{{ __('msg_dt_1.amount') }}:</label>
                                                    <div class="col-sm-9">
                                            ...
                                            <div class="modal-footer"> 
                                                <button type="button" class="btn btn-lh m-auto" @click="set_money();">{{ __('msg_dt_1.confirm_trans_in') }}</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="modal fade mem-modal" id="get_money" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="display: none;">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-header text-white brown">
                                                <h5 class="modal-title" id="get_monty_title">{{ __('msg_dt_1.trans_out') }}</h5>
                                            ...
                                            <div class="modal-footer"> 
                                                <button type="button" class="btn btn-lh m-auto" @click="get_money();">{{ __('msg_dt_1.confirm_trans_out') }}</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
