<!--<div class="container footer">
	<div class="partner_title">{{ __('msg_dt_1.partner') }}</div>
	...
	<div class="copyright text-white text-center">
	    <p>{{ __('msg_dt_1.copyright_desc') }}<br>
	    {{ __('msg_dt_1.copyright') }}</p>
	</div>
</div>-->
...
<div class="footer" id="footer">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header text-white brown">
                    <h5 class="modal-title" id="show_message_title">{{ __('msg_dt_1.my_info') }}</h5>
                ...
                <div class="modal-footer">
                    <button v-if="message_count != 0" type="button" class="btn btn-lh m-auto"  @click="into_message">{{ __('msg_dt_1.check_message') }}</button>
                    <button v-if="url == ''" type="button" class="btn btn-lh m-auto"  data-dismiss="modal">{{ __('msg_dt_1.ok') }}</button>
                    <button v-else type="button" class="btn btn-lh m-auto"  data-dismiss="modal" @click="into_spcific_page(url)">{{ __('msg_dt_1.ok') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="content">
        <div class="partner_subtitle">BEST PARTNER</div>
        <div class="partner-logo"><span class="sr-only">BEST PARTNER Logo</span></div>
        <div class="copyright text-white text-center">
                <p>{{ __('msg_dt_1.copyright_desc') }}<br>
                {{ __('msg_dt_1.copyright') }}</p>
        </div>
    ...
    var login_bar = new Vue({
        ...
        filters : {
            formatDate : function(value){
                ...
                var weekday = ['<?php echo __('msg_dt_1.Sun'); ?>', '<?php echo __('msg_dt_1.Mon'); ?>', '<?php echo __('msg_dt_1.Tus'); ?>', '<?php echo __('msg_dt_1.Wed'); ?>', '<?php echo __('msg_dt_1.Thu'); ?>', '<?php echo __('msg_dt_1.Fri'); ?>', '<?php echo __('msg_dt_1.Sat'); ?>'];
                this.week = weekday[new Date().getDay()];
                return month + '<?php echo __('msg_dt_1.month'); ?>' + day + '<?php echo __('msg_dt_1.day'); ?> ' + hours + ':' + minutes + ':' + seconds + ' ' + this.week;
        ...
        methods: {
            ...
            into_movie: function(){
                ...
                .then(function (response) {
                    ...
                    if(this.json['code'] == '1'){
                        //如果已登入
                        $('#movie_action').attr('href', "http://av.dtg1688.net/");
                        document.getElementById('movie_action').click();
                    }else{
                        self.alertinfo('<?php echo __("msg_dt_1.login_first"); ?>', '/mb/login_page');
                    }
                })
    ...
    var notic_bar = new Vue({
        ...
        methods: {
            get_new_notic: function(){
                ...
                var ApiURL = API_Host + '/system/notic/0/1';
                Vue.prototype.$http = axios;
                ...
                .then(function (response) {
                    ...
                    if(this.json['code'] == '1'){
                        if(this.json['info'].length > 0){
                            self.notic_data = this.json['info'][0]['content'];
                        }
                    }else{
                        self.alertinfo(this.json['info']);
                        return;
