        ...
        <div id="show_records">
            <main role="main" class="container main bank-main">
                <section class="btn-group w-100 funds-menu mb-2" role="group">
                    <div class="btn-group w-30" role="group">
                        <button id="btnGroupDrop-funds" type="button" class="btn btn-secondary dropdown-toggle c-alpha-gold mr-1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" ref="game_name">
                            <i class="fas fa-dice"></i> {{ __('msg_dt_1.game') . __('msg_dt_1.platform') }}
                        </button>
                    ...
                    <div class="btn-group w-30" role="group">
                        <button id="btnGroupDrop-type" type="button" class="btn btn-secondary dropdown-toggle c-alpha-gold mr-1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" ref="game_type">
                            <i class="fas fa-gamepad"></i> {{ __('msg_dt_1.game') . __('msg_dt_1.type') }}
                        </button>
                        <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                            <a class="dropdown-item" href="#" @click="change_game_type('nodata', '{{ __('msg_dt_1.game') . __('msg_dt_1.type') }}')">{{ __('msg_dt_1.game') . __('msg_dt_1.type') }}</a>
                            <a class="dropdown-item" href="#" @click="change_game_type('SX', '{{ __('msg_dt_1.real_casino') }}')">{{ __('msg_dt_1.real_casino') }}</a>
                            <a class="dropdown-item" href="#" @click="change_game_type('DZ', '{{ __('msg_dt_1.slots') }}')">{{ __('msg_dt_1.slots') }}</a>
                            <a class="dropdown-item" href="#" @click="change_game_type('TY', '{{ __('msg_dt_1.sport') }}')">{{ __('msg_dt_1.sport') }}</a>
                            <a class="dropdown-item" href="#" @click="change_game_type('CP', '{{ __('msg_dt_1.lottery') }}')">{{ __('msg_dt_1.lottery') }}</a>
                            <a class="dropdown-item" href="#" @click="change_game_type('QP', '{{ __('msg_dt_1.cardgame') }}')">{{ __('msg_dt_1.cardgame') }}</a>
                            <a class="dropdown-item" href="#" @click="change_game_type('BY', '{{ __('msg_dt_1.fishing').__('msg_dt_1.game') }}')">{{ __('msg_dt_1.fishing').__('msg_dt_1.game') }}</a>
                        </div>
                    </div>
                    ...
                    <div class="btn-group w-30" role="group">
                        <button id="btnGroupDrop-date" type="button" class="btn btn-secondary dropdown-toggle c-alpha-gold" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" ref="game_date">
                            <i class="far fa-calendar-alt"></i> {{ __('msg_dt_1.select_time') }}
                        </button>
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="btnGroupDrop1">
                            <!--<a class="dropdown-item" href="#" @click="change_date('0', '{{ __('msg_dt_1.all_time') }}')">{{ __('msg_dt_1.all_time') }}</a>-->
                ...
                <section class="mb-2 sum-data">
                    <div class="row">
                        <div class="col text-left"><i class="far fa-list-alt"></i> {{ __('msg_dt_1.total_count') }}: <strong class="text-primary">@{{ data_count }}</strong></div>
                        <div class="col text-right"><i class="fas fa-coins"></i> {{ __('msg_dt_1.total_money') }}: <strong class="text-success">@{{ formatMoney(data_bet) }}</strong></div>
                    </div>
                </section>
                <section class="funds-table mb-2" v-if="hasdata === 1" v-for="gamedata in gamedatas">
                    <div class="row funds-date">
                        <div class="col-7">{{ __('msg_dt_1.time') }} : <span>@{{ gamedata.betDay }}</span></div>
                        <div class="col-5 text-right">{{ __('msg_dt_1.total_amount') }} : <strong class="text-warning">@{{ gamedata.settleamount }}</strong></div>
                    </div>
                    <div class="row funds-1">
                        <div class="col">{{ __('msg_dt_1.note_no') }} : <span class="order">@{{ gamedata.orderId }}</span></div>
                    </div>
                    <div class="row funds-1">
                        <div class="col">{{ __('msg_dt_1.game') }} :<span>@{{ gamedata.gamePlatform }}</span>-<span>@{{ gamedata.gameBigType }}-<span>@{{ gamedata.gamename }}</span><!---<strong>游戏名</strong>--></div>
                    </div>
                    <div class="row funds-2">
                        <div class="col-6">{{ __('msg_dt_1.betting') . __('msg_dt_1.amount') }} : <strong class="text-warning">@{{ formatMoney(gamedata.betMoney) }}</strong></div>
                        <div class="col-6 text-right">{{ __('msg_dt_1.winning') }} : <strong :class="[gamedata.intwin == 1 ? 'text-win' : 'text-lose']">@{{ formatMoney(gamedata.netMoney) }}</strong></div>
                    </div>
                </section>
                ...
                <button type="submit" class="btn btn-danger btn-block c-alpha-gold bank-btn mb-3" v-if="bln_no_more == 1" @click="add_count"><i class="fas fa-sync-alt"></i> {{ __('msg_dt_1.load_more') }}</button>
            </main>
        </div>
         <!--go to top-->
            ...
            var app = new Vue({
                ...
                created: function(){
                    ...
                    setTimeout(function(){
                        ...
                        if(session_data != ''){
                            self.change_date('1', '{{ __("msg_dt_1.today") }}');
                            self.get_game_type();
                        }
                    }, 500);
                                ...
                                if(rtnData['count'] > 0){
                                    ...
                                    $.each(rtnData['record'], function(key, value){
                                        ...
                                        switch(value['gamebigtype']){
                                            case 'SX':
                                                bigType = '{{ __("msg_dt_1.real_casino") }}';
                                                break;
                                            case 'DZ':
                                                bigType = '{{ __("msg_dt_1.slots") }}';
                                                break;
                                            case 'TY':
                                                bigType = '{{ __("msg_dt_1.sport") }}';
                                                break;
                                            case 'CP':
                                                bigType = '{{ __("msg_dt_1.lottery") }}';
                                                break;
                                            case 'QP':
                                                bigType = '{{ __("msg_dt_1.cardgame") }}';
                                                break;
                                            case 'BY':
                                                bigType = '{{ __("msg_dt_1.fishing").__("msg_dt_1.game") }}';
                                                break;
                                        }
