            ...
            <main role="main" class="container main account-main">
                ...
                <section class="mt-3">
                    <ul class="row d-flex flex-wrap justify-content-between" v-if="loading_game_finishing == 1">
                        <li class="platform c-alpha-black3 ZBLIVE" v-for="(game,index) in games" >
                            <h6 class="name">@{{ game.name }}</h6>
                            <div class="amount">@{{ formatMoney(game.point) }}</div>
                            <button type="button" class="btn mx-0 login-btn" @click="fast_set_money(game.gametype,index)">{{ __('msg_dt_1.flash_in') }}</button>
                            <button type="button" class="btn mx-0 login-btn" data-toggle="modal" data-target="#set_money" :data-stuff="[game.name,game.gametype]" @click="set_game_index(index)">{{ __('msg_dt_1.trans_in') }}</button>
                            <button type="button" class="btn mx-0 login-btn" data-toggle="modal" data-target="#get_money" :data-stuff="[game.name,game.gametype]" @click="set_game_index(index)">{{ __('msg_dt_1.trans_out') }}</button>
                        </li>
                    </ul>
