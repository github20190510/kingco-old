        ...
        <div id="mb_bankcards">
            <main role="main" class="container main bank-main">
                <section class="d-flex flex-wrap justify-content-around">
                    <div class="add-card">
                        <button type="button" class="btn d-flex align-items-center justify-content-center" onclick="location.href='/mb/bank_card_add'">{{ __('msg_dt_1.add_bank_card_mobile') }}<i class="fas fa-plus-circle ml-3"></i></button>
                    </div>
                    <div class="new-card c-alpha-gold enabled d-flex flex-column" v-for="card_data in card_datas">
                        <div class="card-data">
                            <div class="bank-name"><i class="fas fa-university"></i>@{{card_data.bankname}}</div>
                            <div class="sub-bank">{{ __('msg_dt_1.openning_bank') }}: @{{card_data.openningbank}}</div>
                        ...
                        <div class="edit-bar d-flex justify-content-center mt-auto">
                            <!--<span class="btn btn-sm edit mr-1" data-toggle="modal" data-target="#bankModal">編輯</span>
                            <span class="btn btn-sm delete mr-1">刪除</span>-->
                            <span class="btn btn-sm default ml-auto" v-if="card_data.status == 1 && card_data.infomationcomment == ''" @click="set_default(card_data.informationcode)">{{ __('msg_dt_1.set_preset') }}</span>
                        </div>
