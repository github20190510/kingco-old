            ...
            <main role="main" class="container main bank-main">
                ...
                <section class="funds-table mb-3" v-if="bln_has_save_data == 1" v-for="savedata in savedatas">
                    <div class="row funds-date">
                        <div class="col">{{ __('msg_dt_1.date') }} : <span>@{{savedata.orderdate}}</span></div>
                    </div>
                    <div class="row funds-2">
                        <div class="col">{{ __('msg_dt_1.order_no') }} : <strong class="text-danger">@{{savedata.ordernumber}}</strong></div>
                    </div>
                    <div class="row funds-2">
                        <div class="col-6">{{ __('msg_dt_1.amount') }} : <strong class="text-danger">@{{ formatMoney(savedata.orderamount) }}</strong></div>
                        <div class="col-6 text-right">{{ __('msg_dt_1.status') }} : 
                            <span :class="{'text-success':savedata.orderstatus == 2,'text-danger':savedata.orderstatus == 4}" >@{{savedata.str_status}}</span>
                        </div>
                    </div>
                </section>
                            ...
                            if(this.json['code'] == '1'){
                                ...
                                if(rtnData['count'] != 0){
                                    ...
                                    $.each(rtnData['record'], function(idx, v){
                                        v['str_status'] = self.get_order_status(v['orderstatus'], '{{ __("msg_dt_1.processing") }}', '{{ __("msg_dt_1.processed") }}', '{{ __("msg_dt_1.turn_down") }}', '{{ __("msg_dt_1.refuse") }}', '{{ __("msg_dt_1.pending_payment") }}');
                                    });
