	...
	@case('md_6_0_002')
		<div id="login_bar">
			...
			<nav class="nav login-bar fixed-top d-none d-lg-none d-xl-block">
				...
				<div class="form-inline float-right mr-3 p-3">
					...
					<button v-if="login_status === 0" id="login_btn" @click="chk_login" type="submit" class="btn login_btn my-1 mr-2">{{ __('msg_zy_1.immediately_login_now') }}</button>
					<button v-if="login_status === 0" type="submit" class="btn register_btn mb-1 mr-2 mt-1" @click="register">{{ __('msg_zy_1.free_register') }}</button>
					<button v-if="login_status === 0" type="submit" class="btn forget_btn mb-1 mt-1" @click="alert_forgetpwd">{{ __('msg_zy_1.forget_pwd') }}</button>
			...
			<nav class="navbar navbar-expand-xl fixed-top nav-header navbar-light menu-bar">
				...
				<div class="ml-auto login-box flex-lg-nowrap">
					<button type="button" class="btn login-btn d-xl-none" @click="register" v-if="login_status === 0" >{{ __('msg_zy_1.register') }}</button>
					<button type="button" class="btn login-btn d-xl-none" @click="login" v-if="login_status === 0">{{ __('msg_zy_1.login_mobile') }}</button>
				...
				<button id="menu" class="navbar-toggler menu-btn p-0" type="button" data-toggle="offcanvas"> <span class="navbar-toggler-icon"></span> </button>
			...
			@if(session('model_type') === 'md_6_0_002_mobile')
