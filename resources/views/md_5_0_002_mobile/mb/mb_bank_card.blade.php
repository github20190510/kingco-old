    ...
    <body class="main-body">
        ...
        <header class="login-header navbar-dark">
            <div class="d-flex align-items-center">
                ...
                <div class="header-title m-auto">{{ __('msg_zy_1.bank_card') }}</div>
            </div>
        </header>
        <div id="mb_bankcards">
            <main role="main" class="container main bank-main">
                <section class="d-flex flex-wrap justify-content-center">
                    <div class="add-card">
                        <button type="button" class="btn d-flex align-items-center justify-content-center" onclick="location.href='/mb/bank_card_add'">{{ __('msg_zy_1.add_bank_card_mobile') }}<i class="fas fa-plus-circle ml-5 fa-3x"></i></button>
                    </div>
                    <div class="new-card c-alpha-gold enabled" v-for="card_data in card_datas">
                        <div class="card-data">
                            <div class="bank-name"><i class="fas fa-university"></i> @{{card_data.bankname}}</div>
                            <div class="sub-bank">{{ __('msg_zy_1.openning_bank') }}: @{{card_data.openningbank}}</div>
                        ...
                        <div class="edit-bar d-flex justify-content-center">
                            ...
                            <span class="btn btn-sm default" v-if="card_data.status == 1 && card_data.infomationcomment == ''" @click="set_default(card_data.informationcode)">{{ __('msg_zy_1.set_preset') }}</span>
                        </div>
                        <div class="use-card" v-if="card_data.infomationcomment == '<?php echo __("msg_zy_1.default"); ?>'"><span></span><!--預設卡--></div>
                    </div>
                </section>
            </main>
        </div>
