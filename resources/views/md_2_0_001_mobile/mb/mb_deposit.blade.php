                ...
                <form class="needs-validation" novalidate onSubmit="return false;">
                    <section class="input-group register-input dropdown-box">
                        <div class="input-group-prepend dropdown-left">
                            <label class="input-group-text c-alpha-gold text-white border-0" for="inputGroupSelect01"><i class="fas fa-bars fa-fw mr-1"></i>{{ __('msg_dt_1.select_payment_type') }}</label>
                        </div>
                        <select class="custom-select bank-select" id="inputGroupSelect01" v-model="select_payment_type" >
                            <option  class="text-center">{{ __('msg_dt_1.select_thirdpartys_type') }}...</option>
                            <option value="offline" v-if="if_have_bind_bank_card != undefined">{{ __('msg_dt_1.offline_payment_type') }}</option>
                            <option value="online">{{ __('msg_dt_1.online_payment_type') }}</option>
                        </select>
                    </section>
                    <div v-if="select_payment_type == 'offline'">
                        <section class="input-group register-input dropdown-box">
                            ...
                            <div class="input-group-prepend dropdown-left">
                                <label class="input-group-text c-alpha-gold text-white border-0" for="inputGroupSelect01"><i class="fas fa-globe fa-fw mr-1"></i> {{ __('msg_dt_1.select_payment_way') }}</label>
                            </div>
                            
                            <select class="custom-select bank-select" id="inputGroupSelect01" v-model="offline_index" @change="show_money()">
                                <option value="" class="text-center">{{ __('msg_dt_1.select_thirdpartys_type') }}...</option>
                    ...
                    <div v-if="select_payment_type == 'online'">
                        <section class="input-group register-input dropdown-box">
                            ...
                            <!--<div class="input-group-append dropdown-right">
                                <button class="btn btn-outline-secondary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="btnBank">{{ __('msg_dt_1.select_thirdpartys_type') }}</button>
                    ...
                    <section class="input-group register-input money-box">
                        <div class="input-group-prepend dropdown-left">
                            <label class="input-group-text c-alpha-gold text-white border-0" for="inputGroupSelect2"><i class="fas fa-sort-amount-up fa-fw mr-1 text-white"></i> {{ __('msg_dt_1.money_limit') }}</label>
                        </div>
                        <input type="text" class="form-control bg-transparent bank-imit" :placeholder="strMoneyMsg" readonly="readonly" >
                    </section>
                    <section class="input-group register-input money-box">
                        <div class="input-group-prepend dropdown-left">
                            <label class="input-group-text c-alpha-gold text-white border-0" for="inputGroupSelect01"><i class="fas fa-dollar-sign fa-fw mr-1 text-white"></i> {{ __('msg_dt_1.save_amount') }}</label>
                        </div>
                    ...
                    <section class="mb-3">
                        <div class="row d-flex flex-wrap justify-content-start m-auto money-btn ">
                            <button type="button" class="btn c-alpha-gold" @click="fast_save(50)">50{{ __('msg_dt_1.dollars') }}</button>
                            <button type="button" class="btn c-alpha-gold" @click="fast_save(100)">100{{ __('msg_dt_1.dollars') }}</button>
                            <button type="button" class="btn c-alpha-gold" @click="fast_save(200)">200{{ __('msg_dt_1.dollars') }}</button>
                            <button type="button" class="btn c-alpha-gold" @click="fast_save(500)">500{{ __('msg_dt_1.dollars') }}</button>
                            <button type="button" class="btn c-alpha-gold" @click="fast_save(1000)">1000{{ __('msg_dt_1.dollars') }}</button>
                            <button type="button" class="btn c-alpha-gold" @click="fast_save(2000)">2000{{ __('msg_dt_1.dollars') }}</button>
                            <button type="button" class="btn c-alpha-gold" @click="fast_save(5000)">5000{{ __('msg_dt_1.dollars') }}</button>
                            <button type="button" class="btn c-alpha-gold" @click="fast_save(10000)">10000{{ __('msg_dt_1.dollars') }}</button>
                            <a style="display:none" href="#" ref="pay_link" target="_blank"></a>
                        </div>
                    </section>
                    <button type="submit" class="btn btn-danger btn-block c-alpha-gold bank-btn mb-3" @click="confirm_save">{{ __('msg_dt_1.confirm_ok') }}</button>
                </form>
            </main>
        </div>
        <!--go to top-->
