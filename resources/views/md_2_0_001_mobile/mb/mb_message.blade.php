            ...
            <main role="main" class="container main bank-main" style="height:360px">
                <section class="btn-group w-100 funds-menu mb-3" role="group">
                    ...
                    <div class="btn-group w-30" role="group">
                        <button id="btnGroupDrop-type" type="button" class="btn btn-secondary dropdown-toggle c-alpha-gold mr-1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="read_status" ref="read_status">
                            <i class="far fa-eye"></i> {{ __('msg_dt_1.status') }}
                        </button>
                        <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                            <a class="dropdown-item" href="#" @click="show_read(0, '{{ __('msg_dt_1.all') }}')">{{ __('msg_dt_1.all') }}</a>
                            <a class="dropdown-item" href="#" @click="show_read(2, '{{ __('msg_dt_1.read') }}')">{{ __('msg_dt_1.read') }}</a>
                            <a class="dropdown-item" href="#" @click="show_read(1, '{{ __('msg_dt_1.unread') }}')">{{ __('msg_dt_1.unread') }}</a>
                        </div>
                    </div>
                    <div class="btn-group w-40" role="group">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="btnGroupDrop1">
                            <!--<a class="dropdown-item" href="#" @click="change_date('nodata', '{{ __('msg_dt_1.information') . __('msg_dt_1.time') }}')">{{ __('msg_dt_1.information') . __('msg_dt_1.time') }}</a>-->
                ...
                <section class="funds-table mb-3" v-for="user_msg_data in user_msg_datas" v-if="has_mb_msg_data == 1">
                    <div class="row funds-date">
                        <div class="col">{{ __('msg_dt_1.date') }} : <span>@{{ user_msg_data.text.sendtime }}</span></div>
                    </div>
                    ...
                    <div class="row funds-2">
                        <div class="col">{{ __('msg_dt_1.status') }} : <strong class="text-warning">{{ user_msg_data.readstatus == '2' ? '<?php echo __("msg_dt_1.read"); ?>': '<?php echo __("msg_dt_1.unread"); ?>' }}</strong></div>
                    </div>
                </section>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content c-alpha-white  border-0">
                        <div class="modal-header text-white c-gold border-0">
                            <h6 class="modal-title" id="show_message_title">{{ __('msg_dt_1.trans_in') }}</h6>
                        ...
                        <div class="modal-footer border-0"> 
                            <button type="button" class="btn btn-lh m-auto btn-dark c-gold border-0" id="btnOK" @click="close_msg_dialog();">{{ __('msg_dt_1.ok') }}</button>
                        </div>
                    </div>
                </div>
            </div>
