<div id="menu">
	<nav class="navbar navbar-expand-md fixed-top navbar-dark c-alpha-grey nav-header"> <a class="navbar-brand lh-logo" href="/"></a>
		<div class="ml-auto login-box">
			<button type="button" class="btn text-white login-btn" @click="register" v-if="login_status === '0'">{{ __('msg_dt_1.register') }}</button>
			<button type="button" class="btn text-white login-btn" @click="login" v-if="login_status === '0'">{{ __('msg_dt_1.login_mobile') }}</button>
		</div>
    ...
    <div class="user-bar d-flex align-items-center justify-content-between" v-if="login_status === '1'">
        ...
        <button type="button" class="btn text-white ml-auto mx-0 login-btn" @click="logout('head', '{{ __('msg_dt_1.logout_succ') }}')">{{ __('msg_dt_1.logout_mobile') }}</button>
    </div>
</div>
