...
<html lang="{{ app()->getLocale() }}">
    ...
    <body>
        ...
        <div id="cq9list">
           <!--  <section class="container-fluid text-center slot-nav-bar">
                <div class="slot-type d-flex align-items-center justify-content-center">
                    <button type="button" class="btn slot-type-btn"> <input type="checkbox" checked autocomplete="off"> {{ __('msg_dt_1.prize_pool') . __('msg_dt_1.game') }}</button>
                    <button type="button" class="btn slot-type-btn active">{{ __('msg_dt_1.all') . __('msg_dt_1.game') }}</button>
                    <button type="button" class="btn slot-type-btn">{{ __('msg_dt_1.hot') . __('msg_dt_1.game') }}</button>
                    <button type="button" class="btn slot-type-btn">{{ __('msg_dt_1.classic_slot_machine') }}</button>
                    <button type="button" class="btn slot-type-btn">{{ __('msg_dt_1.horn_slot_machine') }}</button>
                    <button type="button" class="btn slot-type-btn">{{ __('msg_dt_1.video_poker') }}</button>
                    <button type="button" class="btn slot-type-btn">{{ __('msg_dt_1.video_poker') }}</button>
                    <button type="button" class="btn slot-type-btn">{{ __('msg_dt_1.scratch') }}</button>
                </div>
                <div class="input-group input-group-sm search-btn">
                    <input type="text" class="form-control bg-transparent" placeholder="{{ __('msg_dt_1.search'). __('msg_dt_1.game') }}" aria-label="Recipient's username" aria-describedby="button-addon2" v-model="search_word">
            ...
            <main role="main" class="container-fluid main slot-main">
                <div class="game-box d-flex flex-wrap align-content-start justify-content-center">
                    <div class="game flex-column enabled" v-for="(v, k) in gamedatas">
                        <div class="cover">{{ __('msg_dt_1.in_maintenance') }}</div>
                        <div class="gameImg"><!--<a :href="'/game/intogame/CQ9Game/' + v['gamecode']" target="_blank">--><img class="load"  alt="" :src="'{{asset('/css/')}}/' + MODEL_NAME + v['img']" @click="into_game('CQ9Game', v['gamecode'], '{{ __('msg_dt_1.click_into_game_button') }}', '{{ __('msg_dt_1.into_game') }}')"><!--</a>--></div>
                        <div class="gameName">@{{v['gamename']}}</div>
                    </div>
                </div>
            </main>
            <a :href="game_href" target="_blank" ref="gamelink" id="gamelink" style="display:none"></a>
        ...
        readTextFile('{{asset("js/games/cq9_list.js")}}', function(text){

            json_data = JSON.parse(text);
            for_data = json_data['cq9_game_data'];
            var app = new Vue({
                el : '#cq9list',
                data : {
                    gamedatas : new Array(),                    
