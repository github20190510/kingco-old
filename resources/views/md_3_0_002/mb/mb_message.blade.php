...
<html lang="{{ app()->getLocale() }}">
    ...
    <body>
        ...
        <div class="main membe-box" id="mb_message">
            <div class="content">
                                ...
                                <div class="user-edit-module">
                                    <div class="msg-tab">
                                        <ul class="nav nav-tabs" id="myTab" role="tablist">
                                            <li class="nav-item">
                                                <a class="nav-link d-flex align-items-center active" id="home-tab" data-toggle="tab" href="#home1" role="tab" aria-controls="home" aria-selected="true"><i class="far fa-comment-dots mr-1"></i> {{ __('msg_zy_1.my_info') }}</a>
                                            </li>
                                            <li class="nav-item" style="display:none">
                                                <a class="nav-link d-flex" id="profile-tab" data-toggle="tab" href="#profile" role="tab" aria-controls="profile" aria-selected="false"><i class="fas fa-gift mr-1"></i> {{ __('msg_zy_1.day_by_day') . __('msg_zy_1.backwater') }}</a>
                                            </li>
                                            <li class="nav-item">
                                                <a class="nav-link d-flex align-items-center" id="contact-tab" data-toggle="tab" href="#contact" role="tab" aria-controls="contact" aria-selected="false"><i class="fas fa-bullhorn mr-1"></i> {{ __('msg_zy_1.system') . __('msg_zy_1.notification') }}</a>
                                            </li>
                                        </ul>
                                        <div class="tab-content" id="myTabContent">
                                            ...
                                            <div class="tab-pane fade show active" id="home1" role="tabpanel" aria-labelledby="home-tab">
                                                <div>
                                                    <div class="form-inline mb-2 mt-3">
                                                        <label for="inputPassword" class="my-1 mr-2">{{ __('msg_zy_1.date') }}：</label>
                                                        <select class="custom-select my-1 mr-sm-2 custom-select-sm" id="gamedate" @change="change_date" v-model="mdate">
                                                            <option value="nodata" selected>{{ __('msg_zy_1.information') . __('msg_zy_1.time') }}</option>
                                                            <!--<option value="0">{{ __('msg_zy_1.all2') . __('msg_zy_1.time') }}</option>-->
                                                            <option value="1">{{ __('msg_zy_1.today') }}</option>
                                                            <option value="2">{{ __('msg_zy_1.yesterday') }}</option>
                                                            <option value="3">{{ __('msg_zy_1.last_week') }}</option>
                                                            <option value="4">{{ __('msg_zy_1.last_30_days') }}</option>
                                                        </select>
                                                        <button type="button" id="notread_btn" class="btn btn-outline-warning btn-sm" v-if="readstatus === 'nodata'" @click="show_read(1)">{{ __('msg_zy_1.just_unread') }}</button>
                                                        <button type="button" id="all_msg_btn" class="btn btn-outline-warning btn-sm" v-if="readstatus !== 'nodata'" @click="show_read(0)">{{ __('msg_zy_1.view_all') }}</button>
                                                    </div>
                                                    <table class="table table-hover">
                                                        <thead>
                                                            <tr>
                                                                <th scope="col" class="text-left">{{ __('msg_zy_1.date') }}</th>
                                                                <th scope="col">{{ __('msg_zy_1.information') }}</th>
                                                                <th scope="col">{{ __('msg_zy_1.status') }}</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            <tr v-for="user_msg_data in user_msg_datas" v-if="has_mb_msg_data === 1">
                                                                ...
                                                                <td data-toggle="modal" data-target="#show_message" :data-stuff="[user_msg_data.text.content, '{{ __('msg_zy_1.my_info') }}', user_msg_data.messagecode]">@{{ user_msg_data.text.content.substr(0, 30) + '...' }}</td>
                                                                <td>{{ user_msg_data.readstatus == '2' ? '<?php echo __("msg_zy_1.read"); ?>': '<?php echo __("msg_zy_1.unread"); ?>' }}</td>
                                                            </tr>
                                            ...
                                            <div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                                                <div>
                                                    <div class="form-inline mb-2 mt-3">
                                                        <label for="btndate" class="my-1 mr-2">{{ __('msg_zy_1.date') }}：</label>
                                                        <select class="custom-select my-1 mr-sm-2 custom-select-sm" id="btndate">
                                                                <option selected>{{ __('msg_zy_1.betting') . __('msg_zy_1.time') }}</option>
                                                                <option value="0">{{ __('msg_zy_1.all2') . __('msg_zy_1.time') }}</option>
                                                                <option value="1">{{ __('msg_zy_1.today') }}</option>
                                                                <option value="2">{{ __('msg_zy_1.yesterday') }}</option>
                                                                <option value="3">{{ __('msg_zy_1.last_week') }}</option>
                                                    ...
                                                    <table class="table table-hover">
                                                        <thead>
                                                            <tr>
                                                                <th scope="col" class="text-left">{{ __('msg_zy_1.date') }}</th>
                                                                <th scope="col">{{ __('msg_zy_1.game') }}</th>
                                                                <th scope="col">{{ __('msg_zy_1.backwater') . __('msg_zy_1.amount') }}</th>
                                                                <th scope="col">{{ __('msg_zy_1.receive') }}</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            <tr>
                                            ...
                                            <div class="tab-pane fade" id="contact" role="tabpanel" aria-labelledby="contact-tab">
                                                <div>
                                                    <!--<div class="form-inline mb-2 mt-3">
                                                        <label for="betdate" class="my-1 mr-2">{{ __('msg_zy_1.date') }}：</label>
                                                    ...
                                                    <table class="table table-hover">
                                                        <thead>
                                                            <tr>
                                                                <th scope="col" class="text-left">{{ __('msg_zy_1.date') }}</th>
                                                                <th scope="col">{{ __('msg_zy_1.notification') }}</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            <tr v-for="notic_data in notic_datas" v-if="has_notic_data === 1">
                                                                <th scope="row" class="text-left">@{{ notic_data.createtime }}</th>
                                                                <td data-toggle="modal" data-target="#show_message" :data-stuff="[notic_data.content, '{{ __('msg_zy_1.system') . __('msg_zy_1.notification') }}']">@{{ notic_data.content.substr(0, 30) + '...' }}</td>
                                                            </tr>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <button type="button" class="btn btn-lh m-auto" @click="close_msg_dialog" id="btnOK" ref="">{{ __('msg_zy_1.ok') }}</button>
                            </div>
                        </div>
                    </div>
                </div>
