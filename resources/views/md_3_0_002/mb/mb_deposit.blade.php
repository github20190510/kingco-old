...
<html lang="{{ app()->getLocale() }}">
    ...
    <body>
        ...
        <div class="main membe-box" id="mb_deposit">
            <div class="content">
                                            ...
                                            <div class="tips">
                                                <div>
                                                    <div class="mem-info-title">
                                                        <h5 class="d-flex align-items-center"><i class="far fa-credit-card mr-2 fa-fw"></i> {{ __('msg_zy_1.emp_card') }}</h5>
                                                    </div>
                                                    <div class="grid-bd mr-2">
                                                        <div class="row mb-3 d-flex" v-if="if_have_bind_bank_card != undefined">
                                                            ...
                                                            <!-- <div class="col-3 mb-3" v-for="offline_pay in offline_pays">
                                                                <div v-if="offline_pay.qrtype == 1" class="btn btn-light bank-btn" :class="{selected: offline_pay.lsh == thirdid}" @click="change_emp_code(offline_pay.lsh,2,1)"><span class="circle-icon"><i class="fas fa-dollar-sign bank-icon"></i></span>{{ __('msg_zy_1.wechat') }}</div>
                                                                <div v-if="offline_pay.qrtype == 2" class="btn btn-light bank-btn" :class="{selected: offline_pay.lsh == thirdid}" @click="change_emp_code(offline_pay.lsh,2,2)"><span class="circle-icon"><i class="fas fa-dollar-sign bank-icon"></i></span>{{ __('msg_zy_1.alipay') }}</div>
                                                            </div> -->
                                                            <!--
                                                                原本要拿來接Cloud qrcode用的api 後來整合至取銀行卡的api 3/19
                                                            -->
                                                ...
                                                <div>
                                                    <div class="mem-info-title">
                                                        <h5 class="d-flex align-items-center"><i class="far fa-credit-card mr-2 fa-fw"></i> {{ __('msg_zy_1.payment_type') }}</h5>
                                                    </div>
                                                    <div class="grid-bd mr-2">
                                                ...
                                                <div class="mem-info-title mt-0 mb-3 ml-1">
                                                    <h6 class="d-flex align-items-center">{{ __('msg_zy_1.payment_type_escription') }}</h6>
                                                </div>
                                                ...
                                                <div class="money-input">
                                                    <div class="input-group input-group-lg w-50 mb-4">
                                                        <div class="input-group-prepend">
                                                            <span class="input-group-text deposit-label" id="inputGroup-sizing-lg">$ {{ __('msg_zy_1.save_amount') }}</span>
                                                        </div>
                                                        <input type="number" class="form-control mem_input" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-lg" placeholder="{{ __('msg_zy_1.input_amount') }}" v-model="save_money">
                                                        <button type="button" class="btn btn-warning px-5" @click="confirm_save">{{ __('msg_zy_1.submit') }}</button>
                                                        <!--v-on:keyup.13="confirm_save"-->
                                                    </div>
                                                </div>
                                                <div class="money-select">
                                                    <div class="quick-input d-flex flex-wrap">
                                                        <div id="cash-50" class="btn btn-light cash-btn rounded-0" role="button" aria-pressed="true" @click="fast_save(50)" :class="{selected: click_mnoney == 50}">50{{ __('msg_zy_1.dollars') }}</div>
                                                        <div id="cash-100" class="btn btn-light cash-btn rounded-0" role="button" aria-pressed="true" @click="fast_save(100)" :class="{selected: click_mnoney == 100}">100{{ __('msg_zy_1.dollars') }}</div>
                                                        <div id="cash-200" class="btn btn-light cash-btn rounded-0" role="button" aria-pressed="true" @click="fast_save(200)" :class="{selected: click_mnoney == 200}">200{{ __('msg_zy_1.dollars') }}</div>
                                                        <div id="cash-500" class="btn btn-light cash-btn rounded-0" role="button" aria-pressed="true" @click="fast_save(500)" :class="{selected: click_mnoney == 500}">500{{ __('msg_zy_1.dollars') }}</div>
                                                        <div id="cash-1000" class="btn btn-light cash-btn rounded-0" role="button" aria-pressed="true" @click="fast_save(1000)" :class="{selected: click_mnoney == 1000}">1000{{ __('msg_zy_1.dollars') }}</div>
                                                        <div id="cash-2000" class="btn btn-light cash-btn rounded-0" role="button" aria-pressed="true" @click="fast_save(2000)" :class="{selected: click_mnoney == 2000}">2000{{ __('msg_zy_1.dollars') }}</div>
                                                        <div id="cash-5000" class="btn btn-light cash-btn rounded-0" role="button" aria-pressed="true" @click="fast_save(5000)" :class="{selected: click_mnoney == 5000}">5000{{ __('msg_zy_1.dollars') }}</div>
                                                        <div id="cash-10000" class="btn btn-light cash-btn rounded-0" role="button" aria-pressed="true" @click="fast_save(10000)" :class="{selected: click_mnoney == 10000}">10000{{ __('msg_zy_1.dollars') }}</div>
                                                        <a style="display:none" href="#" ref="pay_link" target="_blank"></a>
                                                    </div>
                                                </div>
                                                <!-- <div class="savbtn w-50 text-right">
                                                    <button type="button" class="btn btn-warning px-5" @click="                     this.fast_save_submit();">{{ __('msg_zy_1.fast').__('msg_zy_1.submit') }}</button>
                                                </div> -->
                                            </div>
                                        </div>
                                    </div>
        ...
        <script>
            var mb_deposit = new Vue({
                ...
                data: {
                    ...
                    emp_cards: [],
                    offline_pays: [],
                    strMoneyMsg: '<?php echo __("msg_zy_1.please_select_payment_type"); ?>',
                    bankcode: '',
                    paymentbankcode: '',
                    thirdcode: '',
                    thirdid: '',
                ...
                methods: {
                    ...
                    confirm_amount: function(money){
                        ...
                        if(this.payment_type == 0){
                            ...
                            this.alertinfo('<?php echo __("msg_zy_1.please_select_payment_type"); ?>');
                            return;
                            ...
                            if(this.check_thirdparty_select() == true){
                                if(money >= this.min_money && money <= this.max_money){
                                    //if(confirm('<?php echo __("msg_zy_1.confirm_amount"); ?>' + money + '<?php echo __("msg_zy_1.dollars"); ?>?')){
                    ...
                    go_to_bank_card_or_not: function(){
                        if (!this.if_have_bind_bank_card){
                            this.alertinfo('<?php echo __("msg_zy_1.please_bind_bank_card_first"); ?>');
