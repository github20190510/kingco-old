    ...
    <body class="main-body">
                ...
                <form class="needs-validation" novalidate onSubmit="return false;">
                    <section class="input-group register-input dropdown-box">
                        ...
                        <div class="input-group-append dropdown-right">
                            <button class="btn btn-outline-secondary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="btn_bank">{{ __('msg_fcs_1.select_bank_card') }}</button>
                    ...
                    <div class="input-group register-input">
                        ...
                        <input type="number" class="form-control border-left-0 bg-transparent" id="bankid" placeholder="{{ __('msg_fcs_1.withdrawal') . __('msg_fcs_1.amount') }} : {{ __('msg_fcs_1.input_wd_amount') }}" required v-model="get_money">
                        <div class="invalid-feedback">
                                {{ __('msg_fcs_1.input_wd_amount') }}
                        </div>
                    </div>
                    <div class="input-group register-input">
                        <div class="input-group-append">
                        ...
                        <input type="password" class="form-control border-left-0 bg-transparent" id="password" placeholder="{{ __('msg_fcs_1.pay_pwd') }} : {{ __('msg_fcs_1.input_pay_pass') }}" required v-model="pay_pwd">
                        <div class="invalid-feedback">
                    ...
                    <div class="input-group register-input">
                        ...
                        <textarea class="form-control border-left-0 bg-transparent" id="msg" placeholder="{{ __('msg_fcs_1.mb_comments') }} : {{ __('msg_fcs_1.input_mb_comments') }}" v-model="pay_msg" aria-label="With textarea"></textarea>
                    </div>
                    <div v-if="bln_can_send == 0" style="color:red">{{ __('msg_fcs_1.should_bet') }}：@{{int_should_money}} {{ __('msg_fcs_1.already_bet') }}：@{{int_bet_money}}</div>
        ...
        var mb_withdraw = new Vue({
            ...
            data:{
                ...
                loading_id: 0,
                button_string: '<?php echo __("msg_fcs_1.submit"); ?>',
                bln_can_send: 0,
                int_should_money: 0,
                int_bet_money: 0
            },
            ...
            watch: {
                if_send_or_not: function(val){
                    if (val){
                        var count = 0;
                        mb_withdraw.button_string = '<?php echo __("msg_fcs_1.dealing_with_deposit_order"); ?>';
                        ...
                        mb_withdraw.button_string = '<?php echo __("msg_fcs_1.submit"); ?>';
            ...
            methods: {
                get_money_account: function(){
                    ...
                    if(this.get_money == ''){
                        this.alertinfo('<?php echo __("msg_fcs_1.input_wd_amount"); ?>!');
                        return false;
                    }
                    ...
                    .then(function (response) {
                        ...
                        if(this.json['code'] == '1'){
                            ...
                            self.alertinfo('<?php echo __("msg_fcs_1.waiting_review") ?>', '/mb/mb_index');
                            self.getbagmoney();
                        }else{
                            self.if_send_or_not = false;
                    ...
                    .then(function (response) {
                        ...
                        if(this.json['code'] == '1'){
                            if(this.json['info'].length == 0){
                                self.alertinfo('<?php echo __("msg_fcs_1.no_binding_card"); ?>!', '/mb/bank_card');
                                return;
                            }else{
                                rtn_data = {};
                                $.each(this.json['info'], function(k, v){
                                    if(v['status'] == '1'){
                                        ...
                                        if(self.bank_value == ''){
                                            ...
                                            if(v['infomationcomment'] == '<?php echo __('msg_fcs_1.default'); ?>'){
                                                self.bank_value = v['informationcode'];
                                            }
                                        }
                                    }
