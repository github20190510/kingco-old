    ...
    <body class="main-body">
        <header class="login-header c-purple fixed-top">
            <div class="d-flex align-items-center">
                ...
                <div class="header-title">{{ __('msg_fcs_1.member_login') }}</div>
            </div>
        </header>    
        <main role="main" class="container main register-main" id="login">
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content c-alpha-white  border-0">
                        <div class="modal-header text-white c-gold border-0">
                            <h6 class="modal-title" id="show_message_title">{{ __('msg_fcs_1.my_info') }}</h6>
                        ...
                        <div class="modal-footer border-0"> 
                            <button v-if="message_count != 0" type="button" class="btn btn-lh m-auto btn-dark c-gold border-0"  @click="into_message">{{ __('msg_fcs_1.check_message') }}</button>
                            <button type="button" class="btn btn-lh m-auto btn-dark c-gold border-0"  @click="into_home">{{ __('msg_fcs_1.ok') }}</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <form class="needs-validation" novalidate onSubmit="return false;">
                <div class="input-group register-input">
                    ...
                    <input type="text" class="form-control border-left-0 bg-transparent" id="account" placeholder="{{ __('msg_fcs_1.input_username') }}" v-model="login_page_acc" ref="login_page_acc" required v-focus>
                    <div class="invalid-feedback">{{ __('msg_fcs_1.input_username') }}</div>
                </div>                
                <div class="input-group register-input">
                    <div class="input-group-append">
                        <span class="input-group-text"><i class="fas fa-lock"></i></span>
                    </div>
                    <input type="password" class="form-control border-left-0 bg-transparent" id="password" placeholder="{{ __('msg_fcs_1.input_password') }}" v-model="login_page_pwd" ref="login_page_pwd" required>
                    <div class="invalid-feedback">{{ __('msg_fcs_1.input_password') }}</div>
                </div> 
                <div class="input-group register-input">
                    <div class="input-group-append">
                        <span class="input-group-text"><i class="fas fa-globe"></i></span>
                    </div>
                    <input type="text" class="form-control border-left-0 bg-transparent code-input" id="code" name="captcha" placeholder="{{ __('msg_fcs_1.captcha') }}" ref="verify" v-model="verify" required>
                    ...
                    <div class="invalid-feedback">
                            {{ __('msg_fcs_1.captcha') }}
                    </div>
                </div>
                <button type="submit" class="btn btn-danger btn-block c-alpha-gold register-btn" @click="chk_page_login">{{ __('msg_fcs_1.login_mobile') }}</button>
                <div class="btn-group btn-block mt-3 new-user" role="group" aria-label="Basic example">
                    <button type="button" class="btn btn-danger w-50 c-alpha-gold register-btn" @click="goto_register">{{ __('msg_fcs_1.register_now_mobile') }}</button>
                    <button type="button" class="btn btn-danger w-50 c-alpha-gold register-btn ml-2 border-radius-r" @click="alert_forgetpwd">{{ __('msg_fcs_1.forget_pwd') }}</button>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content c-alpha-white border-0">
                            ...
                            <div class="modal-footer"> 
                                ...
                                <button type="button" class="btn border-0 btn-dark c-gold" @click="auto_trans_point" id="auto_trans_point">{{ __('msg_fcs_1.auto_trans_point') }}</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        var login = new Vue({
            ...
            created: function(){
                ...
                .then(function (response) {
                    ...
                    if(this.json['code'] == '1'){
                        self.alertinfo('<?php echo __('msg_fcs_1.is_logged'); ?>', '/');
                        return;
                    }
                })
            ...
            methods: {
                chk_page_login: function(){
                    ...
                    if(this.login_page_acc == ''){
                        //this.alertinfo('<?php echo __('msg_fcs_1.acc_empty_error'); ?>');
                        this.$refs.login_page_acc.focus();
                        return;
                    }
                    if(this.login_page_pwd == ''){
                        //this.alertinfo('<?php echo __('msg_fcs_1.pass_empty_error'); ?>!');
                        this.$refs.login_page_pwd.focus();
                        return;
                    }

                    if(this.verify == ''){
                        //this.alertinfo('<?php echo __("msg_fcs_1.captcha_empty_err"); ?>');
                        ...
                        .then(function (response) {
                            ...
                            if(this.json['data'] == 'match'){
                                ...
                                self.alertinfo('<?php echo __("msg_fcs_1.captcha_err"); ?>');
                ...
                call_login: function(){
                    ...
                    .then(function (response) {
                        ...
                        if(this.json['code'] != '1'){
                            ...
                            self.setonline();
                            //self.alertinfo('<?php echo __('msg_fcs_1.login_succ'); ?>!');
                ...
                alert_forgetpwd: function(){
                    ...
                    this.alertinfo('<?php echo __('msg_fcs_1.forget_pwd_desc'); ?>');
                    return;
                    ...
                    if(parseInt(money) != NaN && parseInt(money) > 0){
                        if(this.$refs.dt1.value != ''){
                            ...
                            .then(function (response) {
                                this.json = response.data;
                                if(this.json['code'] == '1'){
                                    self.alertinfo('<?php echo __("msg_fcs_1.trans_in_succ"); ?><?php echo __("msg_fcs_1.click_into_game_button"); ?>');
                                    self.change_msg();
                                }else{
                                    self.alertinfo(this.json['info']);
                                }
                        ...
                        if(confirm('<?php echo __("msg_fcs_1.bag_money_not_enough"); ?>')){
                            this.change_msg();
                        }else{
                ...
                get_user_message: function(){
                    ...
                    .then(function (response) {
                        ...
                        if(this.json['code'] == '1'){
                            ...
                            var text = new Object(); 
                            text.content = '<?php echo  __('msg_fcs_1.please_change_password_regularly'); ?>';
                ...
                change_msg: function(){
                    ...
                    $('#msg_data').html('<?php echo __("msg_fcs_1.click_into_game_button"); ?>');
                    $('#exampleModalLabel').html('<?php echo __("msg_fcs_1.into_game"); ?>');
