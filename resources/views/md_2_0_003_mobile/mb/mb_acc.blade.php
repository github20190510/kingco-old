                ...
                <section class="user-data-bar d-flex flex-row">
                    ...
                    <div class="user-data row flex-grow-1 justify-content-between">
                        <div class="col d-flex flex-column justify-content-between">
                            ...
                            <button type="button" class="btn btn-sm transfer-btn" @click="fast_get_money">{{ __('msg_fcs_1.one_click_out') }}</button>
                        </div>
                        <div class="col d-flex flex-column text-right" id="acc_bag_money">
                            <span>{{ __('msg_fcs_1.account_money') }}</span>
                            <h3 id="acc_money_h3">0.00</h3>
                        </div>
                    </div>
                </section>            
                <section class="btn-group btn-block access-bar" role="group" aria-label="Basic example">
                    <button type="button" class="btn btn-secondary c-alpha-gold mr-1" onclick="location.href='/mb/deposit'">{{ __('msg_fcs_1.saving_money') }}</button>
                    <button type="button" class="btn btn-secondary c-alpha-gold mr-1" onclick="location.href='/mb/withdraw'">{{ __('msg_fcs_1.geting_money') }}</button>
                    <button type="button" class="btn btn-secondary c-alpha-gold" onclick="location.href='/mb/bank_card'">{{ __('msg_fcs_1.bank_card') }}</button>
                </section>
                <section class="mt-3">
                    <ul class="row d-flex flex-wrap justify-content-between"  v-if="loading_game_finishing == 1">
                        <li class="platform c-alpha-gold ZBLIVE" v-for="(game,index) in games">
                            ...
                            <button type="button" class="btn text-white mx-0 login-btn" @click="fast_set_money(game.gametype,index)">{{ __('msg_fcs_1.flash_in') }}</button>
                            <button type="button" class="btn text-white mx-0 login-btn" data-toggle="modal" data-target="#set_money" :data-stuff="[game.name,game.gametype]" @click="set_game_index(index)">{{ __('msg_fcs_1.trans_in') }}</button>
                            <button type="button" class="btn text-white mx-0 login-btn" data-toggle="modal" data-target="#get_money" :data-stuff="[game.name,game.gametype]" @click="set_game_index(index)">{{ __('msg_fcs_1.trans_out') }}</button>
                        </li>
                    </ul>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content c-alpha-white  border-0">
                        ...
                        <div class="modal-header text-white c-gold border-0">
                            <h6 class="modal-title" id="exampleModalLabel">{{ __('msg_fcs_1.trans_in') }}</h6>
                        ...
                        <div class="modal-body mt-4">
                            <div class="form-group row">
                                <label for="money" class="col-3 col-form-label">{{ __('msg_fcs_1.amount') }}:</label>
                                <div class="col-9">
                        ...
                        <div class="modal-footer border-0"> 
                            <button type="button" class="btn btn-lh m-auto btn-dark c-gold border-0" @click="set_money();">{{ __('msg_fcs_1.confirm_trans_in') }}</button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal fade mem-modal" id="get_money" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="display: none;">
                <div class="modal-dialog" role="document">
                    <div class="modal-content c-alpha-white  border-0">
                        ...
                        <div class="modal-header text-white c-gold border-0">
                            <h6 class="modal-title" id="exampleModalLabel">{{ __('msg_fcs_1.trans_out') }}</h6>
                        ...
                        <div class="modal-footer border-0"> 
                            <button type="button" class="btn btn-lh m-auto btn-dark c-gold border-0" @click="get_money();">{{ __('msg_fcs_1.confirm_trans_out') }}</button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    set_money: function(){
                        ...
                        if(gtype != '' && money != '' && money > 0){
                            ...
                            .then(function (response) {
                                this.json = response.data;
                                if(this.json['code'] == '1'){
                                    self.setshowbagmony(formatMoney(this.json['info']['balance']),'<?php echo __("msg_fcs_1.balance"); ?> ');
                                ...
                                // self.get_all_money();
                                // self.getbagmoney(session_data, '<?php echo __("msg_fcs_1.balance"); ?> ', '<?php echo __("msg_fcs_1.account_money"); ?>');
