...
* 進入遊戲遊玩頁面
*/
Vue.prototype.into_game = function (gid, gtype, msg1, msg2, dialog_name){
    ...
    .then(function (response) {
        ...
        if(this.json['code'] == '1'){
            //確認點數
            self.chk_has_money(msg1, msg2, newPage, dialog_name);
        }else{
            //導到登入頁
            newPage.close();
...
* newPage : 新頁面選取器
*/
Vue.prototype.chk_has_money = function (msg1, msg2, newPage, dialog_name){
...
Vue.prototype.change_msg = function (msg2, msg5){
    ...
    $('#trans_point_page_m').hide();
    $('#auto_trans_point').hide();
    $('#auto_trans_point_m').hide();
    $('#msg_data').html(msg2);
    $('#msg_data_m').html(msg2);
    $('#exampleModalLabel').html(msg5);
    $('#exampleModalLabel_m').html(msg5);
...
* 為彩票網客製的取遊戲資料部份
**/
Vue.prototype.getBGGame = function(){
    ...
    for(inti = 0; inti < cnt; inti++ ){
        ...
        if(arrData.hasOwnProperty(for_data[inti]['gamecate']) == false){
            arrData[for_data[inti]['gamecate']] = new Array();
        }

        arrData[for_data[inti]['gamecate']].push({'gamecode' : for_data[inti]['gamecode'], 'gamename' : game_lang_name, 'img': imgUrl, 'status': for_data[inti]['status']});
